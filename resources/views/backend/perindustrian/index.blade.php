...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Data Perindustrian</h1>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="card card-primary">
                    <div class="card-header">
                        ...
                        <div class="card-tools">
                            <a href="{{url('perindustrian-import')}}">
                            ...
                            <table id="list-data" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Bentuk</th>
                                        @php
                                        $gettahun = DB::table('data_perindustrian')
                                        ->groupby('tahun')
                                        ->orderby('id','asc')
                                        ->get();
                                        @endphp
                                ...
                                <tbody>
                                    ...
                                    $idua=1;
                                    $headdua =
                                    DB::table('input_perindustrian')->where('header_satu',$row->header_satu)->groupBy('header_dua')->orderby('id','asc')->get();
                                    @endphp
                                    ...
                                    @else
                                    @php
                                    $lastdata = DB::table('data_perindustrian')
                                    ->where('kode',$rowdua->id)
                                    ->orderby('id','desc')
                                    ->limit(1)
                                    ->get();

                                    $alldata = DB::table('data_perindustrian')
                                    ->where('kode',$rowdua->id)
                                    ->orderby('id','asc')
                                    ->get();
                                    @endphp
                                    ...
                                    $itiga=1;
                                    $headtiga =
                                    DB::table('input_perindustrian')->where([['header_satu',$row->header_satu],['header_dua',$rowdua->header_dua]])->groupBy('header_tiga')->orderby('id','asc')->get();
                                    @endphp
                                    @foreach($headtiga as $rowtiga)
                                    @if($rowtiga->header_dua !=$rowtiga->header_tiga)
                                    ...
                                    <tr>
                                        <td class="pl-5">
                                            <span class="ml-3">{{$isatu-1}}.{{$idua-1}}.{{$itiga++}}
                                                {{$rowtiga->header_tiga}}</span>
                                        </td>
                                        <td>
                                            @foreach($lastdata as $last)
                                    ...
                                    $iempat=1;
                                    $headempat =
                                    DB::table('input_perindustrian')->where([['header_satu',$row->header_satu],['header_dua',$rowdua->header_dua],['header_tiga',$rowtiga->header_tiga]])->get();
                                    @endphp
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Hapus Perindustrian</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <form action="{{url('/perindustrian/hapus')}}" method="POST" enctype="multipart/form-data">
