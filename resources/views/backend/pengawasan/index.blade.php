...
<div class="content-wrapper">
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0 text-dark">Data Pengawasan</h1>
                </div>
            </div>
        </div>
    </div>
            ...
            <div class="col-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Data Pengawasan</h3>
                        <div class="card-tools">
                            <a href="{{url('data-pengawasan-import')}}">
                            ...
                            <table id="list-data" class="table table-bordered table-striped">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Bentuk</th>
                                        @php
                                        $gettahun = DB::table('data_pengawasan')
                                        ->groupby('tahun')
                                        ->orderby('id','asc')
                                        ->get();
                                        @endphp
                                ...
                                <tbody>
                                    ...
                                    $idua=1;
                                    $headdua =
                                    DB::table('input_pengawasan')->where('header_satu',$row->header_satu)->orderby('id','asc')->get();
                                    @endphp
                                    @foreach($headdua as $rowdua)
                                    @php
                                    $lastdata = DB::table('data_pengawasan')
                                    ->where('kode',$rowdua->id)
                                    ->orderby('id','desc')
                                    ->limit(1)
                                    ->get();

                                    $alldata = DB::table('data_pengawasan')
                                    ->where('kode',$rowdua->id)
                                    ->orderby('id','asc')
                                    ->get();
                                    @endphp
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title">Hapus Data Pengawasan</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">×</span>
                </button>
            </div>
            <form action="{{url('/data-pengawasan/hapus')}}" method="POST" enctype="multipart/form-data">
