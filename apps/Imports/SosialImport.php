...
class SosialImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = SosialModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_sosial')->count();
    $jumlahdiisi = DB::table('data_sosial')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
    $namafile = 'Data Sosial';
    $pj = 'DINSOS';
