...
class PerdaganganImport implements ToCollection, WithHeadingRow
{
    /**
    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PerdaganganModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_perdagangan')->count();
        $jumlahdiisi = DB::table('data_perdagangan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Perdagangan';
        $pj = 'DISPERINDAG';
