    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = KeuanganModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_keuangan')->count();
        $jumlahdiisi = DB::table('data_keuangan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Keuangan';
        $pj = 'BPKAD';
