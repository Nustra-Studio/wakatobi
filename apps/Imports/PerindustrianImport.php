...
class PerindustrianImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PerindustrianModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_perindustrian')->count();
        $jumlahdiisi = DB::table('data_perindustrian')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Perindustrian';
        $pj = 'DISPERINDAG';
