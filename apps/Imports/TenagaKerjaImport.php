    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = TenagaKerjaModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_tenagakerja')->count();
    $jumlahdiisi = DB::table('data_tenagakerja')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
    $namafile = 'Data Tenaga Kerja';
    $pj = 'DINKOP UKM TK';
