...
class PendidikanImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PendidikanModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_pendidikan')->count();
    $jumlahdiisi = DB::table('data_pendidikan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
    $namafile = 'Data Pendidikan';
    $pj = 'DISPENDIKBUD';
