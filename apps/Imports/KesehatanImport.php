...
class KesehatanImport implements ToCollection, WithHeadingRow
{
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = KesehatanModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_kesehatan')->count();
    $jumlahdiisi = DB::table('data_kesehatan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
    $namafile = 'Data Kesehatan';
    $pj = 'RSUD, DINKES';
