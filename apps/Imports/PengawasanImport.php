...
class PengawasanImport implements ToCollection, WithHeadingRow
{
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PengawasanModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
        ...
        $jumlahdata = DB::table('input_pengawasan')->count();
        $jumlahdiisi = DB::table('data_pengawasan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Pengawasan';
        $pj = 'INSPEKTORAT';
