...
class SekertariatDaerahImport implements ToCollection, WithHeadingRow
{
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = SekertariatDaerahModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
        ...
        $jumlahdata = DB::table('input_sekertariat_daerah')->count();
        $jumlahdiisi = DB::table('data_sekertariat_daerah')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Sekertariat Daerah';
        $pj = 'SEKDA';
