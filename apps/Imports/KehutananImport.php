...
class KehutananImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = KehutananModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_kehutanan')->count();
        $jumlahdiisi = DB::table('data_kehutanan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Kehutanan';
        $pj = 'DISTAN';
