...
class ProgramUnggulanImport implements ToCollection, WithHeadingRow
{
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = ProgramUnggulanModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
        ...
        $jumlahdata = DB::table('input_program_unggulan')->count();
        $jumlahdiisi = DB::table('data_program_unggulan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Program Unggulan Daerah';
        $pj = 'DISKOMINFO SP';
