...
class PariwisataImport implements ToCollection, WithHeadingRow
{
    ...
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PariwisataModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_pariwisata')->count();
        $jumlahdiisi = DB::table('data_pariwisata')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Pariwisata';
        $pj = 'DISPAR';
