...
class PertanahanImport implements ToCollection, WithHeadingRow
{
    /**
    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PertanahanModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_pertanahan')->count();
    $jumlahdiisi = DB::table('data_pertanahan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
    $namafile = 'Data Pertanahan';
    $pj = 'DINKOMINFO SP';
