...
class PanganImport implements ToCollection, WithHeadingRow
{
    /**
    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PanganModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_pangan')->count();
    $jumlahdiisi = DB::table('data_pangan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
    $namafile = 'Data Pangan';
    $pj = 'DKP';
