...
class PerikananImport implements ToCollection, WithHeadingRow
{
    /**
    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = PerikananModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
    ...
    $jumlahdata = DB::table('input_perikanan')->count();
        $jumlahdiisi = DB::table('data_perikanan')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Perikanan';
        $pj = 'DKP';
