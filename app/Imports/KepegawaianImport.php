...
class KepegawaianImport implements ToCollection, WithHeadingRow
{
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = KepegawaianModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
        ...
        $jumlahdata = DB::table('input_kepegawaian')->count();
        $jumlahdiisi = DB::table('data_kepegawaian')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Kepegawaian serta pendidikan & Pelatihan';
        $pj = 'BKPSDM';
