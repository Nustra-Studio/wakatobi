...
class GeografisImport implements ToCollection, WithHeadingRow
{
    /**
    ...
    */
    public function collection(Collection $collection)
    {
        ...
        foreach ($collection as $row){
            $tahun=$row['tahun'];
            $kode = $row['kode'];
            $data = GeografisModel::firstOrNew(['kode' => $kode,'tahun'=>$tahun]);
            $data->kode = $row['kode'];
        ...
        $jumlahdata = DB::table('input_geografis')->count();
        $jumlahdiisi = DB::table('data_geografis')->where([['tahun',$tahunnya],['jumlah','!=',0]])->count();
        $namafile = 'Data Geografis';
        $pj = 'DISKOMINFO SP';
